...
const RegistrationForm = () => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const result = await SponsorCheckout(price ?? "");
      ...
      if (result.data?.checkout_url) {
        ...
        // Add success redirect URL with parameters to the checkout URL
        const successUrl = encodeURIComponent(
          `${window.location.origin}/success?status=success&checkout_id=${result.data.checkout_id}&eagle_id=${encodeURIComponent(formData.eagle_id)}`
        );

        // If your payment provider supports success_url parameter, append it
        const checkoutUrlWithSuccess = `${result.data.checkout_url}&success_url=${successUrl}`;

        window.location.href = checkoutUrlWithSuccess;
      } else {
